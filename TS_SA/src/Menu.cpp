...
void Menu::enable()
{
	...
	while (true)
	{
		...
		case 5:
			...
			{
				std::cout << "Nie podano czasu lub wsp�czynnika dT ";
				cin.get();
			}
			else
			{
                std::cout << "Wybierze sposob chlodzenia:" << endl;
                std::cout << "1. Geometryczne" << endl;
                std::cout << "2. Liniowy" << endl;
                std::cout << "3. Logarytmiczny" << endl;

                cin >> te;

				SimualtedAnnealing sa(graph, time, temperatureFactor, te);
				sa.apply();
				cin.get();
			}
			break;
